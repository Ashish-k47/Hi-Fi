...
export const getOrCreatedConversation = async (req: AuthRequest, res: Response) => {
...
export const sendMessage = async (req: AuthRequest, res: Response) => {
    ...
    if (file) {
        try {
            ...
            const uploadPromise = new Promise<{ secure_url: string }>((resolve, reject) => {
                const uploadStream = cloudinary.uploader.upload_stream({ folder: "HiFi_chat", resource_type: resourceType }, (error, result) => {
    ...
    let conversation;
    if(conversationId) {
        conversation = await Conversation.findOne({_id: conversationId, participants: { $in: [sendedId] }})
    }else {
        conversation = await findConversation(sendedId, receiverId)
        if (!conversation) {
            conversation = await Conversation.create({participants: [sendedId, receiverId]})
        }
    }
    if (!conversation) {
    ...
    const message = await Message.create({
        sender: sendedId,
        receiver: receiverId || conversation.participants.find((p) => String(p) !== sendedId),
...
export const deleteConversation = async (req: AuthRequest, res: Response) => {
    ...
    try {
        ...
        res.json({success: true, message: "Chat delete successfully"});
    } catch (error) {
